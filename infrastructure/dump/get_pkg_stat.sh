...
teamdump=$(mktemp -d /var/trafdump/team${team}_XXXXX)
if [[ $? != 0 ]]; then
    exit 1
fi
...
c=0
for f in $(ls -t | head -${lastfiles}); do
    tcpdump -nr "$f" "net $net1 or net $net2" -w "${teamdump}/${c}" 2>/dev/null
    c=$((c + 1))
done

mergecap -F pcap -w "team${team}.latest.cap" ${teamdump}/*
rm -rf "${teamdump}"

getstat() {
    printf "%-6s:" "$1"
    n=$(tcpdump -Kqnr "team${team}.latest.cap" $2 2>/dev/null | wc -l)
    printf "%-6d " $n
    echo
}
